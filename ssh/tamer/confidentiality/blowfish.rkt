...
@handbook-chunk[<bf-subkey-spec>
  (context "subkeys" #:do
    (it-check-subkeys key-data key-test key-outs))]

@tamer-note[#:note 'code-inset]{subkeys}

@handbook-scenario[#:tag "bf-vector"]{Test Vectors}

@handbook-chunk[<bf-ecb-spec>
  (context "vector" #:do
    (for/spec ([key (in-vector ecb-data)]
               [plaintext (in-vector plain-data)]
               [ciphertext (in-vector cipher-data)])
      (it-check-blowfish/ecb plaintext key ciphertext)))]

@tamer-note[#:note 'code-inset]{vector}

@handbook-scenario[#:tag "bf-cbc"]{Chain mode Test}

@handbook-chunk[<bf-cbc-spec>
  (context "chain mode" #:do
    (it-check-blowfish/cbc #:plaintext cbc-data
                           #:IV cbc-iv #:key cbc-key
                           #:ciphertext cbc-ok))]

@tamer-note[#:note 'code-inset]{chain mode}
...
@chunk[|<*>|
       ...
       (module tamer typed/racket
         ...
         (spec-begin blowfish #:do
                     <bf-subkey-spec>
                     <bf-ecb-spec>
                     <bf-cbc-spec>))]

@chunk[<blowfish-behaviors>
       (define-context (it-check-subkeys keydata keyfull keyouts)
         #:desc ["data[~a]= ~a" (bytes-length keydata) (string-upcase (bytes->hexstring keydata))]
         #:do (for/spec ([size (in-range 1 (bytes-length keyfull))])
                ...
                (it ["c=~a k[~a]=~a"
                     (string-upcase (bytes->hexstring out))
                     (~r size #:min-width 2)
                     (string-upcase (bytes->hexstring key))] #:do
                  (expect-bytes= out (vector-ref keyouts (sub1 size))))))

       (define-behavior (it-check-blowfish/ecb data key ciphertext)
         (let-values ([(encrypt decrypt) (blowfish-cipher key)])
           #:it ["key: ~a\t\tdata: ~a" (string-upcase (bytes->hexstring key)) (string-upcase (bytes->hexstring data))]
           #:do
           (let* ([cout (encrypt data)]
                  [pout (decrypt cout)])
             (expect-bytes= cout ciphertext)
             (expect-bytes= pout data))))
       
       (define-behavior (it-check-blowfish/cbc #:plaintext data #:IV IV #:key key #:ciphertext ciphertext)
         (let*-values ([(cipher-size) (bytes-length ciphertext)]
                       [(plaintext) (plaintext-0pad data blowfish-blocksize)]
                       [(encrypt decrypt) (blowfish-cipher-cbc IV key)])
           #:it ["~s [key: ~s; initial vector: ~s]" plaintext (bytes->hexstring key) (bytes->hexstring IV)]
           #:do
           (let* ([cout (encrypt plaintext)]
                  [pout (decrypt cout)])
             (expect-bytes= (subbytes cout 0 cipher-size) ciphertext)
             (expect-bytes= pout plaintext))))]
...
@chunk[<blowfish>
       ...
       (define key-test
         (bytes #xf0 #xe1 #xd2 #xc3 #xb4 #xa5 #x96 #x87
       ...
       (define ecb-data
         (vector (bytes #x00 #x00 #x00 #x00 #x00 #x00 #x00 #x00)
       ...
       (define cipher-data
         (vector (bytes #x4E #xF9 #x97 #x45 #x61 #x98 #xDD #x78)
       ...
       (define cbc-ok
         (bytes #x6B #x77 #xB4 #xD6 #x30 #x06 #xDE #xE6
