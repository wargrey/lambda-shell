...
@tamer-action[
 (define-values (octets rsa?) (pem-read openssl-rsa.pem #:label 'RSA-Private-Key))
 (define pem (unsafe-bytes->rsa-private-key* octets))
 (and rsa? (rsa-key-okay? pem))
 (rsa-key-display pem 24)
 (define message (file->bytes (digimon-path "stone" "openssl_rsa.pub")))
 (define signature (rsa-sign pem message id-sha1 'emsa-v1.5))
 (bytes=? signature (file->bytes (digimon-path "stone" "openssl_rsa_sha1.sign")))
...
@chunk[<datatype>
       ...
       (require "../digivice/keygen/rsa/keygen.rkt")

       (define openssl-rsa.pem (digimon-path "stone" "openssl_rsa.pem"))
