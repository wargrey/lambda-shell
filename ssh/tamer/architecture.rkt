...
@tamer-action[
 (HMAC testcase1-256 ssh-hmac-sha256 (make-bytes 20 #x0B) #"Hi There")
 (HMAC testcase2-256 ssh-hmac-sha256 #"Jefe" #"what do ya want for nothing?")
 (HMAC testcase3-256 ssh-hmac-sha256 (make-bytes 20 #xAA) (make-bytes 50 #xDD))
 (HMAC testcase4-256 ssh-hmac-sha256 (apply bytes (range #x01 #x1A)) (make-bytes 50 #xCD))
 (HMAC testcase5-256 ssh-hmac-sha256-128 (make-bytes 20 #x0C) #"Test With Truncation")
 (HMAC testcase6-256 ssh-hmac-sha256 (make-bytes 131 #xAA) #"Test Using Larger Than Block-Size Key - Hash Key First")
 (HMAC testcase7-256 ssh-hmac-sha256 (make-bytes 131 #xAA) #"This is a test using a larger than block-size key and a larger than block-size data. The key needs to be hashed before being used by the HMAC algorithm.")]
