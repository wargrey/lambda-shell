...
(define ssh-log-kexinit : (->* (SSH-MSG-KEXINIT String) (Log-Level) Void)
  (lambda [msg prefix [level 'debug]]
    (ssh-log-message level "~a KEX algorithms: ~a" prefix (ssh:msg:kexinit-kex-methods msg))
    (ssh-log-message level "~a host key algorithms: ~a" prefix (ssh:msg:kexinit-key-formats msg))
    (ssh-log-message level "~a c2s encryption algorithms: ~a" prefix (ssh:msg:kexinit-c2s-ciphers msg))
    (ssh-log-message level "~a s2c encryption algorithms: ~a" prefix (ssh:msg:kexinit-s2c-ciphers msg))
    (ssh-log-message level "~a c2s MAC algorithms: ~a" prefix (ssh:msg:kexinit-c2s-mac-algorithms msg))
    (ssh-log-message level "~a s2c MAC algorithms: ~a" prefix (ssh:msg:kexinit-s2c-mac-algorithms msg))
    (ssh-log-message level "~a c2s compression algorithms: ~a" prefix (ssh:msg:kexinit-c2s-compression-methods msg))
    (ssh-log-message level "~a s2c compression algorithms: ~a" prefix (ssh:msg:kexinit-s2c-compression-methods msg))))
